...
let apiKey = "8d50d6b2cd9b8e497fadda98c47df0eb"
    ...
    NSURLConnection.sendAsynchronousRequest(searchRequest, queue: processingQueue) {response, data, error in
      ...
      do {
        ...
        let flickrPhotos : [FlickrPhoto] = photosReceived.map {
            ...
            if let imageData = NSData(contentsOfURL: flickrPhoto.flickrImageURL()) {
                flickrPhoto.thumbnail = UIImage(data: imageData)
                print("thumbnail found: \(flickrPhoto.flickrImageURL())")
            } else {
                print("image not found: \(flickrPhoto.flickrImageURL())")
            }
